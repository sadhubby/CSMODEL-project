...
class RuleMiner(object):
    ...
    def get_confidence(self, data, rule):
        ...
        concat_support = self.get_support(data, flatlist)
        x_support = self.get_support(data, x)

        confidence = concat_support / x_support if x_support != 0 else 0
