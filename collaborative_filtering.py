...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        aligned = vector1_nonan.index.intersection(vector2_nonan.index)
        vec1_aligned = vector1_nonan[aligned]
        vec2_aligned = vector2_nonan[aligned]

        dot_product = vec1_aligned.dot(vec2_aligned)
        norm_vector1 = np.sqrt(np.sum(vec1_aligned ** 2))
        norm_vector2 = np.sqrt(np.sum(vec2_aligned ** 2))
