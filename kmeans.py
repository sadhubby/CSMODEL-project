...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                ...
                distances[j] = data.apply(lambda row: self.get_euclidean_distance(row[self.start_var : self.end_var], centroid), axis = 1)
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        euclidean_distance = (((point2 - point1)**2).sum())**0.5
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            ...
            # Hint: Use the get_euclidean_distance() function that we have
            # defined in this class.
            distances[i] = sliced_data.apply(lambda row: self.get_euclidean_distance(row, self.centroids.iloc[i]), axis=1)
    ...
    def adjust_centroids(self, data, groups):
        ...
        numeric = grouped_data.select_dtypes(include=[np.number])
        centroids = numeric.groupby('group').mean()
        return centroids
